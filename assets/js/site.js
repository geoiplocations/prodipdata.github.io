...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (currentPage === 'country') {
      await renderCountryLandingPage({ releaseMonth, releaseYear });
    }
...
async function renderCountryLandingPage(context) {
  const metricsNode = document.querySelector('[data-country-metrics-target]');
  const largestNode = document.querySelector('[data-country-largest-target]');
  const asnNode = document.querySelector('[data-country-asn-target]');
  const concentrationNode = document.querySelector('[data-country-concentration-target]');

  const [overview, countryProfiles, concentration] = await Promise.all([
    ...
    fetchJson(assetPath(`assets/data/country-concentration-${context.releaseMonth}.json`))
  ]);

  const countries = getItems(countryProfiles)
    .filter(item => numericValue(item.prefixCount) > 0 || numericValue(item.asnCount) > 0);

  const concentrationMap = new Map(
    getItems(concentration).map(item => [String(item.iso2 || '').toUpperCase(), item])
  );

  const totalPrefixes = countries.reduce((sum, item) => sum + numericValue(item.prefixCount), 0);
  const totalIpv4 = countries.reduce((sum, item) => sum + numericValue(item.ipv4Count), 0);
  const totalAsns = countries.reduce((sum, item) => sum + numericValue(item.asnCount), 0);
  const rirCount = new Set(countries.map(item => String(item.rir || '').toLowerCase()).filter(Boolean)).size;
  ...
  const asnDenseCountries = countries
    .slice()
    .sort((a, b) => numericValue(b.asnCount) - numericValue(a.asnCount))
    .slice(0, 8);

  const concentratedCountries = countries
    .map(item => {
      const key = String(item.iso2 || '').toUpperCase();
      const concentrationRecord = concentrationMap.get(key) || {};
      return {
        ...item,
        top1AsnShare: numericValue(concentrationRecord.top1AsnShare),
        top5AsnShare: numericValue(concentrationRecord.top5AsnShare),
        top10AsnShare: numericValue(concentrationRecord.top10AsnShare)
      };
    })
    .filter(item => numericValue(item.asnCount) >= 5 && numericValue(item.top1AsnShare) > 0)
  ...
  if (metricsNode) {
    metricsNode.innerHTML = `
      <div class="metric-card glass">
        <div class="muted">Published countries</div>
        <div class="metric-value">${formatInteger(overview.totalCountriesRepresented || countries.length)}</div>
        <div class="card-text">Country and territory profiles represented in the current monthly release.</div>
      </div>
      <div class="metric-card glass">
        <div class="muted">Observed ASNs</div>
        <div class="metric-value">${formatInteger(overview.totalAsns || totalAsns)}</div>
        <div class="card-text">Autonomous systems represented across the visible country profiles.</div>
      </div>
      <div class="metric-card glass">
        <div class="muted">Published /24 footprint</div>
        <div class="metric-value">${formatCompact(overview.totalPrefixes || totalPrefixes)}</div>
        <div class="card-text">Published prefix footprint behind the country-level release view.</div>
      </div>
      <div class="metric-card glass">
        <div class="muted">Registry coverage</div>
        <div class="metric-value">${formatInteger(overview.totalRirsRepresented || rirCount)}</div>
        <div class="card-text">Regional Internet Registry groups represented in the country dataset.</div>
      </div>
    `;
  }

  if (largestNode) {
    largestNode.innerHTML = largestCountries.length
      ...
      `).join('')
      : renderSimpleEmptyState('Country footprint unavailable', 'No country footprint data is available for the selected release.');
  }

  if (asnNode) {
    asnNode.innerHTML = asnDenseCountries.length
      ...
      `).join('')
      : renderSimpleEmptyState('Country ASN view unavailable', 'No country ASN summary is available for the selected release.');
  }

  if (concentrationNode) {
    concentrationNode.innerHTML = concentratedCountries.length
      ...
      `).join('')
      : renderSimpleEmptyState('Country concentration unavailable', 'No country concentration summary is available for the selected release.');
  }
}

function formatCountryTopAsnLabel(item) {
  const asnId = numericValue(item.topAsnId);
  if (asnId > 0) {
    return item.topAsnName || `AS${Math.trunc(asnId)}`;
  }
